...
//source
//    .buffer(
//        timeSpan: .seconds(2),               // 2초 내에 받은걸 방출함
//        count: 2,                            // MaxCount
...
print("-------window-------") // buffer와 비슷하지만 array가 아님, return값이 observable임
...
print("-------delay-------") // 위에는 구독만을 뒤로 미루는 역할만 하지만, delay는 sequence자체를 미룸
...
print("-------timeout-------") // 시간초과되면 error timeout
